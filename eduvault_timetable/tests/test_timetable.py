...
class TestTimetable(TestTimetableCommon):
    ...
    def test_case_timetable(self):
        session = self.op_session.create({
            'timing_id': self.env.ref('eduvault_timetable.op_timing_1').id,
            ...
            'faculty_id': self.env.ref('eduvault_core.op_faculty_1').id,
            'batch_id': self.env.ref('eduvault_core.op_batch_1').id,
            'subject_id': self.env.ref('eduvault_core.op_subject_1').id
        })
